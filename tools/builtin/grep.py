...
class GrepParams(BaseModel):
    ...
    path: str = Field('.', description='File or directory path to search in (default: current directory)')
    case_insensitive: bool = Field(False, description='Case-insensitive search (default: false)')

class GrepTool(Tool):
    name = 'grep'
    description = (
        'Search for a regex pattern in file contents. Return the matching lines with file paths and line numbers.'
    )
    kind = ToolKind.READ
    schema = GrepParams
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        if not search_path.exists():
            return ToolResult.error_result(
                f"File or directory {search_path} does not exist",
            )
        ...
        for file_path in files:
            ...
            if file_matches:
                output_lines.append("")
        if not output_lines:
            return ToolResult.success_result(f"No matches found for pattern: {params.pattern} in {search_path}",
                                             metadata={
    ...
    def _find_files(self, search_path: Path) -> list[Path]:
        ...
        for root, dirs, filenames in os.walk(search_path):
            dirs[:] = [ d for d in dirs if d not in {'node_modules', '__pycache__', '.git', '.vscode', '.pytest_cache', '.venv', '.env', '.DS_Store'}]
