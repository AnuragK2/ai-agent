from tools.base import Tool, ToolResult, ToolKind, ToolInvocation, FileDiff
from pydantic import BaseModel, Field
from utils.paths import resolve_path, ensure_parent_directory
from pathlib import Path
from tools.base import ToolConfirmation
class EditParams(BaseModel):
    path: str = Field(..., description="The path to the file to edit (relative to working directory or absolute path).")
    old_string: str = Field("", description="The exact text to find and replace. Must match exactly (including whitespace and indentation) and be unique in the file. For new files, leave this empty.")
    new_string: str = Field(..., description="The text to replace the old_string with. Can be empty to delete text.")
    replace_all: bool = Field(False, description="Replace all occurrences of the old_string (default: false)")

class EditTool(Tool):
    name='edit'
    description=(
        "Edit a file by replacing text. You MUST call read_file on the target path "
        "in this conversation before editing, so old_string matches the file exactly "
        "(including whitespace and indentation). old_string must be unique in the file "
    ...
    async def execute(self, invocation:ToolInvocation)->ToolResult:
        ...
        if not path.exists():
            if params.old_string:
                return ToolResult(f'File does not exist: {path}. To create a new file, use an empty old_string.')
            
            ensure_parent_directory(path)
            path.write_text(params.new_string, encoding='utf-8')
            
            line_count=len(params.new_string.splitlines()) or 0
        ...
        if not params.old_string:
            return ToolResult.error_result(f'old_string is empty but file already exists: {path}. Provide an old_string to edit the file or use write_file to overwrite.')
        ...
        if new_content == old_content:
            return ToolResult.error_result(f'No changes made to {path} - old_string equals new_string.')
        
        try:
            path.write_text(new_content, encoding='utf-8')
            line_count=len(new_content.splitlines()) or 0
            diff=FileDiff(path=path, old_content=old_content, new_content=new_content, is_new_file=False)
        except IOError as e:
    ...
    def _no_match_error(self,old_string: str, content: str, path: Path)->ToolResult:
        ...
        if partial_matches:
            ...
            error_msg+=(
                ...
                "- Try re-reading the file using read_file and then editing."
            )
