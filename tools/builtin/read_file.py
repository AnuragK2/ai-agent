...
class ReadFileParams(BaseModel):
    ...
    limit: int | None = Field(
        None,
        ge=1,
        description='Maximum number of lines to read. If not specified, the entire file will be read.'
    )
    
class ReadFileTool(Tool):
    name='read_file'
    description=(
        "Read the context of a text file. Returns the file content with line numbers ."
    ...
    async def execute(self, invocation: ToolInvocation)->ToolResult:
        ...
        if file_size > self.MAX_FILE_SIZE:
            return ToolResult.error_result(f"File is too large to read: {file_size / (1024*1024):.1f} MB.",
        ...
        if is_binary_file(path):
            ...
            return ToolResult.error_result(f"Cannot read binary files: {path.name} ({size_str}).",
        ...
        try:
            ...
            if total_lines==0:
                return ToolResult.success_result(f"File is empty.", metadata={'lines': 0})
            ...
            return ToolResult.success_result(output=output, truncated=truncated, metadata={
                'path': str(path),
