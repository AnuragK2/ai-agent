...
class AgentEvent:
    ...
    def agent_error(cls, error: str, details: dict[str, Any] | None = None) -> "AgentEvent":
        return cls(type=AgentEventType.AGENT_ERROR, data={"error": error, "details": details or {}})
    ...
    def tool_call_start(cls, call_id: str, name: str, arguments: dict[str, Any]) -> "AgentEvent":
        return cls(type=AgentEventType.TOOL_CALL_START, data={"call_id": call_id, "name": name, "arguments": arguments})
    
    @classmethod
    def tool_call_complete(cls, call_id: str, name: str, result: ToolResult) -> "AgentEvent":
        return cls(type=AgentEventType.TOOL_CALL_COMPLETE, data={"call_id": call_id, "name": name, 'success':result.success, 'output':result.output, 'error':result.error, 'metadata':result.metadata, "truncated":result.truncated})