...
class ChatCompactor:
    ...
    def _format_history_for_compaction(self, messages: list[str, Any]) -> str:
        ...
        for msg in messages:
            ...
            if role == 'tool':
                ...
                if msg.get('tool_calls'):
                    ...
                    output.append(f'Assistant used tools:\n'+ "\n".join(tool_details))
            else:
                truncated= content[:1500] if len(content) > 1500 else content
                if len(content) > 1500:
                    truncated+='\n... [message truncated]'
                output.append(f'User response:\n{truncated}')   
    ...
    async def compress(self, context_manager: ContextManager) -> tuple[str | None, TokenUsage | None]:
        ...
        try:
            summary=""
            usage=None
            async for event in self.client.chat.completion(
                compression_messages,
                stream=False,
            ):
